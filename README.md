...
```bash
sudo mkdir -p /opt/servers && curl -fsSL https://github.com/KorenBar/agent-app-server/archive/refs/heads/main.tar.gz | sudo tar -xz --strip-components=1 -C /opt/servers && cd /opt/servers && sudo ./setup-agent-app.sh -d
```
